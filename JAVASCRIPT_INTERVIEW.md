...
3. element diff
需要配置key
3.1 新旧集合中的节点都是相同的节点的情况下

![element_diff_noadd](./img/element_diff_noadd.png "element_diff_noadd")

步骤1:新元素index=0  节点B   节点在原先排列中的oldIndex=1  初始maxIndex=0
结果1:将oldIndex赋值于maxIndex=>maxIndex=1

步骤2:新元素index=1  节点A   节点在原先排列中的oldIndex=0  初始maxIndex=1
结果2:将当前节点移动A到index=1的位置

步骤3:新元素index=2  节点D   节点在原先排列中的oldIndex=3  初始maxIndex=1
结果3:将oldIndex赋值于maxIndex=>maxIndex=3

步骤4:新元素index=3  节点C   节点在原先排列中的oldIndex=2  初始maxIndex=3
结果4:将当前节点移动C到index=3的位置
